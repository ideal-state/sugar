...
import team.idealstate.sugar.bytecode.api.member.JavaPackage;
...
class InternalJavaPackage implements JavaPackage {
    ...
    @NotNull
    static JavaPackage newInstance(@NotNull String packageName) throws BytecodeParsingException {
        ...
        return PACKAGES_CACHE.computeIfAbsent(packageName, name -> {
            ...
            InputStream inputStream = ClassLoader.getSystemResourceAsStream(InternalJavaClass.internalize(className) + ".class");
            ...
            Visitor visitor = new Visitor(InternalJavaClass.ASM_API, null, internalJavaPackage);
    ...
    private static class Visitor extends ClassVisitor {
        ...
        private Visitor(int api, ClassVisitor cv, @NotNull InternalJavaPackage internalJavaPackage) {
            super(api, cv);
